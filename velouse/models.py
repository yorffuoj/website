...
STATION_STATUSES = (("OPEN", "Open"), ("CLOSED", "Closed"))
...
class Station(models.Model):
    ...
    def refresh(
        self, bike_stands, available_bike_stands, available_bikes, status, last_update
    ):
        formatted_update = make_aware(datetime.fromtimestamp(last_update / 1000))
...
class VelouseUser(AbstractUser):
    ...
    def set_map_view(self, zoom, lat, lng):
        ...
        logger.info(
            f"Set default map viem to {map_center} at zoom {zoom} for user {self.username}"
        )
...
def user_created(sender, instance, created, **kwargs):
    if created and not instance.map_center:
        pos, creation = Position.objects.get_or_create(
            lat=settings.DEFAULT_LATITUDE, lng=settings.DEFAULT_LONGITUDE
        )
